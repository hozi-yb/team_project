...
solar_plant_df["Color(hex)"] = solar_plant_df["power generation capacity(kWh)"].apply(lambda capacity: colormap1(norm(capacity)))
...
facility_df['normalized_count'] = facility_df['facility_count'].apply(lambda count: colormap1(norm(count)))
...
facility_df['normalized_capacity'] = facility_df['facility_capacity'].apply(lambda capacity: colormap1(norm(capacity)))
